...
class BookCommentServiceImpl(
    ...
    @Transactional
    override fun insert(request: UpsertBookCommentRequest): BookComment {
        return save(request.text, request.bookId)
    }

    @Transactional
    override fun update(id: Long, request: UpsertBookCommentRequest): BookComment {
        return save(request.text, request.bookId, id)
    }
